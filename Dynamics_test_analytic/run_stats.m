...
[mean_old_beta,unc_old_beta,std_old_beta,mean_young_beta,unc_young_beta,std_young_beta] = ttest_plot(old_betas, young_betas, 0.95, 'Betas by Subject')
[mean_old_sigma,unc_old_sigma,std_old_sigma,mean_young_sigma,unc_young_sigma,std_young_sigma] = ttest_plot(old_sigmas, young_sigmas, 0.95, 'Sigmas by Subject')
%%----------------------------------------------------------------------%%
%% Below is draft code
%% Load Data
...
% plot_histogram([],round(sqrt(length(old_betas))),log(old_betas))
xlim([-1000 12200])
title('Older Subjects Damping Symmetric')
xlabel('Eigenvalue Product')
subplot(2,2,2)
plot_histogram([],round(sqrt(length(old_sigmas))),old_sigmas)
% plot_histogram([],round(sqrt(length(old_sigmas))),log(old_sigmas))
xlim([800 8200])
title('Older Subjects Damping Asymmetric')
xlabel('Eigenvalue Product')
% Younger subjects
young = load('eigen_bestparams.mat');
young_betas = young.DampingSymYoung(:,1);
young_sigmas = young.DampingAsymYoung(:,1);
...
%%
old = load('subs_bestparams_updated_duarte_old_20230724.mat');
young = load('subs_bestparams_updated_duarte_young_20230724.mat');
...
% ttest_plot(old_betas, young_betas, 0.95, '\beta by Subjects')
% ttest_plot(old_sigmas,young_sigmas,0.95,'\sigma_r by Subjects')
ttest_plot(old_betas, young_betas, 0.95, 'Betas by Subject')
ttest_plot(old_sigmas, young_sigmas, 0.95, 'Sigmas by Subject')
% ttest_plot(log(old_sigmas_new),log(young_sigmas_new),0.95,'Principle Eigenvector Angle by Subjects')
% legend('older','younger')
%%
zIP = [];
for f = 1:length(old.Frequency)
